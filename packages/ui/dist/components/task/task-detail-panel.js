...
export function TaskDetailPanel({ taskId, open, onOpenChange, task, isLoading = false, availableTags = [], onUpdate, onDelete, onAssignTag, onRemoveTag, onShare, renderSubtaskList, renderComments, renderAttachments, renderActivity, renderAssigneeSelector, renderCreateTagDialog, labels = {}, }) {
    ...
    return (_jsxs(Sheet, { open: open, onOpenChange: onOpenChange, children: [_jsxs(SheetContent, { side: "right", className: "w-full sm:max-w-xl md:max-w-2xl p-0 gap-0 overflow-hidden flex flex-col bg-background transition-colors", hideCloseButton: true, children: [_jsx(SheetTitle, { className: "sr-only", children: task?.title || t.title }), isLoading ? (_jsxs("div", { className: "flex flex-col h-full animate-pulse p-6 space-y-4", children: [_jsx(Skeleton, { className: "h-8 w-1/3" }), _jsx(Skeleton, { className: "h-32 w-full" }), _jsx(Skeleton, { className: "h-64 w-full" })] })) : (_jsxs("div", { className: "flex flex-col h-full animate-in fade-in duration-300", children: [_jsxs("div", { className: "px-4 sm:px-6 py-4 sm:py-5 border-b bg-muted/10", children: [_jsxs("div", { className: "flex items-center justify-between gap-2 mb-3 sm:mb-0 sm:absolute sm:right-4 sm:top-4 sm:z-10", children: [_jsxs("div", { className: "flex items-center gap-1 sm:hidden", children: [_jsxs(Select, { value: formData.status, onValueChange: (val) => {
                                                            handleFieldChange('status', val);
                                                            if (onUpdate && taskId)
                                                                onUpdate(taskId, { status: val });
                                                        }, children: [_jsx(SelectTrigger, { className: "h-7 text-xs w-auto gap-1 border-transparent bg-secondary/50 hover:bg-secondary/80 focus:ring-0", children: _jsx(SelectValue, {}) }), _jsx(SelectContent, { children: Object.entries(STATUS_CONFIG).map(([key, config]) => (_jsx(SelectItem, { value: key, children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: cn('w-2 h-2 rounded-full', config.color) }), config.label] }) }, key))) })] }), _jsxs(Select, { value: formData.priority, onValueChange: (val) => {
                                                            handleFieldChange('priority', val);
                                                            if (onUpdate && taskId)
                                                                onUpdate(taskId, { priority: val });
                                                        }, children: [_jsx(SelectTrigger, { className: "h-7 text-xs w-auto gap-1 border-transparent bg-secondary/50 hover:bg-secondary/80 focus:ring-0", children: _jsx(SelectValue, {}) }), _jsx(SelectContent, { children: Object.entries(PRIORITY_CONFIG).map(([key, config]) => (_jsx(SelectItem, { value: key, children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx(config.icon, { className: cn('w-3 h-3', config.textColor) }), config.label] }) }, key))) })] })] }), _jsxs("div", { className: "flex items-center gap-1", children: [isEditing ? (_jsxs(Button, { size: "sm", onClick: handleSave, disabled: isUpdating, className: "h-8 text-xs", children: [_jsx(Save, { className: "w-3.5 h-3.5 sm:mr-1.5" }), _jsx("span", { className: "hidden sm:inline", children: t.btnSave })] })) : (_jsxs(_Fragment, { children: [_jsx(Button, { variant: "ghost", size: "icon", onClick: () => setIsEditing(true), className: "h-8 w-8", children: _jsx(Layout, { className: "w-4 h-4 text-muted-foreground" }) }), _jsx(Button, { variant: "ghost", size: "icon", onClick: handleShare, className: "h-8 w-8", children: _jsx(Share2, { className: "w-4 h-4 text-muted-foreground" }) })] })), _jsx(Button, { variant: "ghost", size: "icon", onClick: () => onOpenChange(false), className: "h-8 w-8", children: _jsx(X, { className: "w-4 h-4 text-muted-foreground" }) })] })] }), _jsx("div", { className: "sm:flex sm:items-start sm:justify-between sm:gap-6 sm:pr-28", children: _jsxs("div", { className: "flex-1 space-y-3 sm:space-y-4", children: [_jsxs("div", { className: "hidden sm:flex items-center gap-2", children: [_jsxs(Select, { value: formData.status, onValueChange: (val) => {
                                                                handleFieldChange('status', val);
                                                                if (onUpdate && taskId)
                                                                    onUpdate(taskId, { status: val });
                                                                ...
                                                                borderColor: tag.color + '40'
                                                                        ...
                                                                        if (taskId && onRemoveTag)
                                                                            onRemoveTag(taskId, tag.id);
                                                                                ...
                                                                                }, className: "w-full flex items-center gap-2 px-2 py-2 text-sm rounded-md hover:bg-accent hover:text-accent-foreground text-primary transition-colors cursor-pointer font-medium", children: [_jsx(Plus, { className: "w-4 h-4" }), _jsx("span", { children: t.tagsCreate })] })] }) })] })] })] }) })] }), _jsx("div", { className: "flex-1 overflow-y-auto overflow-x-hidden", children: _jsxs("div", { className: "p-4 sm:p-6 space-y-4 sm:space-y-6 min-w-0", children: [_jsxs("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6", children: [_jsxs("div", { className: "md:col-span-2 space-y-3 flex flex-col min-w-0", children: [_jsx(Label, { className: "text-xs font-semibold text-muted-foreground uppercase tracking-wider", children: t.descriptionLabel }), isEditing ? (_jsx(Textarea, { value: formData.description, onChange: (e) => handleFieldChange("description", e.target.value), placeholder: t.descriptionPlaceholder, className: "flex-1 resize-none text-sm" })) : (_jsx("div", { className: cn("prose prose-sm dark:prose-invert max-w-none flex-1 p-3 sm:p-4 rounded-lg border-2 border-dashed transition-all overflow-hidden", formData.description
                                                                ? "border-border/50 bg-muted/20 hover:border-border hover:bg-muted/30"
                                                                : "border-border/30 bg-muted/10 hover:border-border/50 hover:bg-muted/20", "cursor-text"), onClick: () => setIsEditing(true), children: formData.description ? (_jsx("p", { className: "whitespace-pre-wrap text-sm leading-relaxed break-words", children: formData.description })) : (_jsx("p", { className: "text-muted-foreground italic text-sm", children: t.descriptionEmpty })) }))] }), _jsx("div", { className: "space-y-4 min-w-0", children: _jsxs("div", { className: "space-y-4 p-4 sm:p-5 rounded-xl bg-gradient-to-br from-muted/40 to-muted/20 border-2 border-border/50 shadow-sm", children: [_jsxs("h3", { className: "font-bold text-sm mb-3 flex items-center gap-2", children: [_jsx("div", { className: "w-1 h-4 bg-primary rounded-full flex-shrink-0" }), t.detailsTitle] }), _jsxs("div", { className: "space-y-4", children: [_jsxs("div", { className: "space-y-2", children: [_jsx(Label, { className: "text-xs font-medium text-muted-foreground", children: t.dueDate }), _jsxs("div", { className: "flex items-center gap-2 p-2 rounded-lg bg-background/60 border border-border/50 hover:border-border transition-colors", children: [_jsx(Calendar, { className: "w-4 h-4 text-muted-foreground flex-shrink-0" }), _jsx(Input, { type: "date", value: formData.dueDate, onChange: (e) => {
