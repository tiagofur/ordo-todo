import { MarkAsReadUseCase, MarkAsReadInput } from "../usecase/mark-as-read.usecase";
...
describe("MarkAsReadUseCase", () => {
  ...
  const validInput: MarkAsReadInput = {
  ...
  describe("execute()", () => {
    ...
    it("should be idempotent - allow marking already read notification", async () => {
      const readNotification = mockNotification.markAsRead();
      mockRepository.findById.mockResolvedValue(readNotification);
      ...
      // update should still be called even if already read
      expect(mockRepository.update).toHaveBeenCalledWith(result);
    });
    ...
    it("should trim whitespace from userId", async () => {
      ...
      // Should still find the notification and verify ownership
      expect(mockRepository.findById).toHaveBeenCalledWith("notif-123");
    });
