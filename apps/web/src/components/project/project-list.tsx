...
import { Loader2, CheckCircle2, Circle, Calendar, Flag } from "lucide-react";
        ...
        <div className="divide-y divide-border">
          {tasks?.map((task: any) => (
            ...
            >
              ...
              <div className={cn("text-xs font-medium px-2 py-1 rounded-full bg-muted", priorityColors[task.priority as keyof typeof priorityColors])}>
                {task.priority}
              </div>
            </div>
          ))}
          ...
          {tasks?.length === 0 && (
            <div className="p-8 text-center text-muted-foreground">
              No tasks found. Add one in the Board view!
            </div>
          )}
        </div>
      </div>
