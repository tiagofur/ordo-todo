...
export function TasksView() {
  ...
  const activeTagIds = filters.tagIds || (urlTagId ? [urlTagId] : []);

  // Filter tasks by tag if present (for those not filtered by server yet)
  let filteredTasks = activeTagIds.length > 0 && tasks
    ? tasks.filter((task: any) =>
        activeTagIds.some(tid => task.tags?.some((tag: any) => tag.id === tid))
      )
    : tasks;

  // Apply remaining filters (status, priority, date)
  if (filteredTasks && hasActiveFilters) {
    filteredTasks = applyFilters(filteredTasks);
  }

  // Get the first active tag for the UI header info
  const firstTagId = activeTagIds[0];
  const currentTag =
    firstTagId && tags ? tags.find((tag: any) => tag.id === firstTagId) : null;

  const clearTagFilter = () => {
    const newFilters = { ...filters };
    delete newFilters.tagIds;
    // Also clear legacy tag param
