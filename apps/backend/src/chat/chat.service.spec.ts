...
describe('ChatService', () => {
    ...
    beforeEach(async () => {
        ...
        chatRepository = module.get<ChatRepository>(ChatRepository) as jest.Mocked<ChatRepository>;
    ...
    describe('createConversation', () => {
        it('should create a conversation without initial message', async () => {
            ...
            const result = await service.createConversation(mockUserId, { title: 'Test' });
        ...
        it('should create conversation and send initial message', async () => {
            ...
            const mockMessage = { id: 'msg-1', content: 'Hello', role: ChatRole.USER, createdAt: new Date() };
            const mockAIResponse = { id: 'msg-2', content: 'Hi!', role: ChatRole.ASSISTANT, createdAt: new Date(), metadata: null };
            ...
            chatRepository.getConversation.mockResolvedValue({ ...mockConversation, title: null } as any);
    ...
    describe('getConversations', () => {
        it('should return paginated conversations', async () => {
            ...
            const result = await service.getConversations(mockUserId, { limit: 20, offset: 0 });

            expect(chatRepository.getConversations).toHaveBeenCalledWith(mockUserId, { limit: 20, offset: 0 });
    ...
    describe('getConversation', () => {
        it('should return conversation with messages', async () => {
            const mockConversation = {
                ...
                context: null,
                messages: [
                    { id: 'msg-1', role: ChatRole.USER, content: 'Hello', createdAt: new Date(), metadata: null },
                ],
                isArchived: false,
                createdAt: new Date(),
                updatedAt: new Date(),
            ...
            const result = await service.getConversation(mockConversationId, mockUserId);
        ...
        it('should throw NotFoundException when conversation not found', async () => {
            ...
            await expect(service.getConversation('non-existent', mockUserId))
    ...
    describe('sendMessage', () => {
        it('should send message and receive AI response', async () => {
            const mockUserMessage = { id: 'msg-1', content: 'Hello', role: ChatRole.USER, createdAt: new Date() };
            const mockAIMessage = { id: 'msg-2', content: 'Hi!', role: ChatRole.ASSISTANT, createdAt: new Date(), metadata: null };
            ...
            chatRepository.getConversation.mockResolvedValue({ title: 'Existing' } as any);
            coachService.chat.mockResolvedValue({
                message: 'Hi!',
                actions: [],
                suggestions: [],
            ...
            const result = await service.sendMessage(mockConversationId, mockUserId, { message: 'Hello' });
        ...
        it('should throw ForbiddenException when user does not own conversation', async () => {
            ...
            await expect(
                service.sendMessage(mockConversationId, mockUserId, { message: 'Hello' }),
    ...
    describe('archiveConversation', () => {
        it('should archive conversation when owned', async () => {
            ...
            expect(chatRepository.archiveConversation).toHaveBeenCalledWith(mockConversationId, mockUserId);
        });

        it('should throw ForbiddenException when not owned', async () => {
            chatRepository.verifyOwnership.mockResolvedValue(false);

            await expect(service.archiveConversation(mockConversationId, mockUserId))
                .rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('getInsights', () => {
        it('should return proactive insights', async () => {
            const mockInsights = [
                { type: 'OVERDUE_ALERT', message: 'You have overdue tasks', priority: 'HIGH' as const, actionable: true },
            ];
            coachService.getProactiveInsights.mockResolvedValue(mockInsights);

            const result = await service.getInsights(mockUserId);

            expect(coachService.getProactiveInsights).toHaveBeenCalledWith(mockUserId);
