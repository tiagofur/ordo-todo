...
export class PrismaSearchRepository implements SearchRepository {
  ...
  async search(query: SearchQuery): Promise<SearchResults> {
    ...
    const searchResults = result.results.map(
      (r) =>
        new SearchResult({
          id: r.id,
          type: r.type,
  ...
  async getSuggestions(
    ...
    Array<{ text: string; type: 'query' | 'task' | 'project'; count: number }>
  > {
    const suggestions = await this.semanticSearchService.getSuggestions(
      userId,
      partialQuery,
    );
    return suggestions
      .slice(0, limit)
      .map((s) => ({ text: s, type: 'query', count: 0 }));
  }
