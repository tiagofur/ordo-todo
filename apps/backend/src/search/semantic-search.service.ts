...
export class SemanticSearchService {
    ...
    async search(
        ...
        if (searchTypes.includes('task')) {
            searchPromises.push(this.searchTasks(userId, interpretation, filters, options));
        }
        if (searchTypes.includes('project')) {
            searchPromises.push(this.searchProjects(userId, interpretation, filters, options));
        }
        if (searchTypes.includes('habit')) {
            searchPromises.push(this.searchHabits(userId, interpretation, filters, options));
        }
...
Responde ÚNICAMENTE con JSON:
{
  ...
  "explanation": "Breve explicación de cómo interpretaste la búsqueda"
        ...
        try {
            const response = await this.geminiAI.generate(systemPrompt, `Query: "${query}"`);
    ...
    private parseQueryLocally(query: string): SearchInterpretation {
        ...
        if (lowerQuery.includes('urgente') || lowerQuery.includes('urgent')) priorities.push('URGENT');
        if (lowerQuery.includes('alta') || lowerQuery.includes('importante')) priorities.push('HIGH');
        if (lowerQuery.includes('baja') || lowerQuery.includes('opcional')) priorities.push('LOW');

        // Detect statuses
        const statuses: string[] = [];
        if (lowerQuery.includes('pendiente') || lowerQuery.includes('por hacer')) statuses.push('TODO');
        if (lowerQuery.includes('en progreso') || lowerQuery.includes('trabajando')) statuses.push('IN_PROGRESS');
        if (lowerQuery.includes('completada') || lowerQuery.includes('terminada') || lowerQuery.includes('hecha')) statuses.push('DONE');
        ...
        if (lowerQuery.includes('vencida') || lowerQuery.includes('atrasada')) dates.push('overdue');
    ...
    private extractKeywords(query: string): string[] {
        const stopWords = new Set([
            ...
            'a', 'y', 'o', 'mi', 'mis', 'tu', 'tus', 'su', 'sus',
            'buscar', 'encontrar', 'mostrar', 'ver', 'listar',
            'tareas', 'tarea', 'proyecto', 'proyectos',
        ]);
    ...
    private async searchTasks(
        ...
        return tasks.map(task => ({
            ...
            relevanceScore: this.calculateRelevance(task.title, task.description || '', keywords),
            highlights: this.extractHighlights(task.title, task.description || '', keywords),
            metadata: {
                status: task.status,
    ...
    private async searchProjects(
        ...
        return projects.map(project => ({
            ...
            relevanceScore: this.calculateRelevance(project.name, project.description || '', keywords),
            highlights: this.extractHighlights(project.name, project.description || '', keywords),
            metadata: {
    ...
    private async searchHabits(
        ...
        return habits.map(habit => ({
            ...
            relevanceScore: this.calculateRelevance(habit.name, habit.description || '', keywords),
            highlights: this.extractHighlights(habit.name, habit.description || '', keywords),
            metadata: {
     ...
     */
    private calculateRelevance(title: string, description: string, keywords: string[]): number {
        ...
        for (const keyword of keywords) {
            ...
            if (lowerTitle.includes(keyword)) {
                ...
                if (lowerTitle.startsWith(keyword) || lowerTitle.includes(` ${keyword}`)) {
                    score += 20;
                }
            }
            // Description matches
     ...
     */
    private extractHighlights(title: string, description: string, keywords: string[]): string[] {
        const highlights: string[] = [];

        for (const keyword of keywords) {
            // Check title
            if (title.toLowerCase().includes(keyword)) {
                highlights.push(`Título: ...${this.highlightKeyword(title, keyword)}...`);
            }
            // Check description
     ...
     */
    private getSnippetAround(text: string, keyword: string, contextLength: number): string {
     ...
     */
    async ask(userId: string, question: string): Promise<{
        answer: string;
        data?: any;
        ...
        const [totalTasks, pendingTasks, overdueTasks, completedToday] = await Promise.all([
...
Responde con JSON:
{
  ...
  "data": null
        ...
        try {
            const response = await this.geminiAI.generate(systemPrompt, `Pregunta: "${question}"`);
            const jsonMatch = response.match(/\{[\s\S]*\}/);
