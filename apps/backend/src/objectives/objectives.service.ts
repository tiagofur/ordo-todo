...
export class ObjectivesService {
  ...
  async findAll(
    ...
    if (options?.workspaceId) {
      objectives = objectives.filter(
        (o) => o.props.workspaceId === options.workspaceId,
      );
    }
  ...
  async findCurrentPeriod(userId: string): Promise<Objective[]> {
    ...
    return objectives.filter(
      (o) =>
        (o.props.status === 'ACTIVE' ||
          (o.props.status as any) === 'AT_RISK') &&
  ...
  async getDashboardSummary(userId: string) {
    ...
    const summary = {
      ...
      completed: objectives.filter((o) => o.props.status === 'COMPLETED')
        .length,
      atRisk: objectives.filter((o) => (o.props.status as any) === 'AT_RISK')
        .length,
      averageProgress:
        objectives.length > 0
          ? Math.round(
  ...
  async update(
    ...
    const updatedObjective = objective.clone({
      ...
      startDate: updateDto.startDate
        ...
        : objective.props.startDate,
      endDate: updateDto.endDate
        ? new Date(updateDto.endDate)
        : objective.props.endDate,
  ...
  async updateKeyResult(
    ...
    const keyResult =
      await this.objectiveRepository.findKeyResultById(keyResultId);
  ...
  async linkTask(keyResultId: string, linkDto: LinkTaskDto, userId: string) {
    ...
    await this.objectiveRepository.linkTask(
      keyResultId,
      linkDto.taskId,
      linkDto.weight,
    );
