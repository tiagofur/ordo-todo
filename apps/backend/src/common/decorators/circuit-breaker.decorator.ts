    ...
    descriptor.value = async function (...args: any[]) {
      ...
      if (circuit.state === CircuitBreakerState.OPEN) {
        if (
          ...
          (mergedOptions.resetTimeout ?? 10000)
        ) {
          circuit.state = CircuitBreakerState.HALF_OPEN;
          logger.warn(`Circuit ${circuitKey} transitioned to HALF_OPEN`);
        } else {
          logger.error(
            `Circuit ${circuitKey} is OPEN. Blocking request to prevent cascading failure.`,
          );
          throw new Error(
            `Circuit ${circuitKey} is currently OPEN due to repeated failures`,
          );
        }
      }
      ...
      try {
        ...
        if (circuit.state === CircuitBreakerState.HALF_OPEN) {
          ...
          if (circuit.successes >= (mergedOptions.successThreshold ?? 2)) {
            ...
            circuit.successes = 0;
            logger.log(
              `Circuit ${circuitKey} transitioned back to CLOSED after ${mergedOptions.successThreshold} successes`,
            );
          }
        ...
        if (circuit.state === CircuitBreakerState.CLOSED) {
          if (circuit.failures >= (mergedOptions.failureThreshold ?? 5)) {
            ...
            logger.error(
              `Circuit ${circuitKey} transitioned to OPEN after ${circuit.failures} failures`,
            );
          }
        } else if (circuit.state === CircuitBreakerState.HALF_OPEN) {
          circuit.state = CircuitBreakerState.OPEN;
          logger.error(
            `Circuit ${circuitKey} transitioned back to OPEN after failure in HALF_OPEN`,
          );
        }
