...
export class BurnoutPreventionService {
    ...
    async getRestRecommendations(userId: string): Promise<RestRecommendation[]> {
        ...
        if (
            ...
            recommendations.push({
                type: 'END_DAY',
                priority: 'HIGH',
                message: 'Es tarde. El descanso nocturno es crucial para tu productividad mañana.',
        ...
        if (
            ...
            analysis.patterns.weekendWorkPercentage > this.THRESHOLDS.warningWeekendWork
        ) {
            recommendations.push({
                type: 'WEEKEND_OFF',
                priority: 'MEDIUM',
                message: 'Los fines de semana sin trabajo mejoran tu productividad semanal.',
        ...
        if (
            ...
            recommendations.push({
                ...
                suggestedAction: 'Revisa tus tareas y delega o pospone las menos urgentes',
            });
        }
    ...
    async checkForIntervention(userId: string): Promise<Intervention | null> {
        ...
        if (analysis.riskLevel === 'MODERATE' && analysis.warnings.length >= 2) {
            return {
                ...
                message: 'Has estado trabajando bastante. Recuerda que el descanso también es productivo.',
     ...
     */
    async generateWeeklyWellbeingSummary(userId: string): Promise<WellbeingSummary> {
        ...
        for (let i = 1; i < sortedSessions.length; i++) {
            const prevEnd = new Date(sortedSessions[i - 1].startedAt).getTime() +
    ...
    private generateWarnings(patterns: WorkPatternAnalysis): BurnoutWarning[] {
        ...
        if (patterns.lateNightPercentage > this.THRESHOLDS.warningLateNight) {
            warnings.push({
                ...
                recommendation: 'Intenta terminar tu trabajo antes de las 10pm para mejor descanso',
            });
        } else if (patterns.lateNightPercentage > this.THRESHOLDS.maxHealthyLateNight) {
            warnings.push({
                type: 'LATE_NIGHTS',
                severity: 'MILD',
        ...
        if (patterns.weekendWorkPercentage > this.THRESHOLDS.warningWeekendWork) {
            warnings.push({
                ...
                recommendation: 'Los fines de semana libres mejoran la productividad semanal',
            });
        } else if (patterns.weekendWorkPercentage > this.THRESHOLDS.maxHealthyWeekendWork) {
            warnings.push({
                type: 'WEEKEND_WORK',
                severity: 'MILD',
                message: `Trabajaste ${patterns.weekendDays} días de fin de semana`,
                recommendation: 'Reserva al menos un día del fin de semana para descansar',
            });
        }

        // Session length warning
        if (patterns.longestSessionWithoutBreak > this.THRESHOLDS.warningSessionWithoutBreak) {
            warnings.push({
                type: 'NO_BREAKS',
                severity:
        ...
        if (patterns.avgDailyWorkHours > this.THRESHOLDS.warningDailyHours) {
            warnings.push({
                ...
                recommendation: 'Limita tu jornada a 8 horas para mantener la productividad',
            });
        }
    ...
    private async calculateTaskLoadTrend(
        ...
        const change = previousTasks > 0 ? (recentTasks - previousTasks) / previousTasks : 0;
