...
export class TeamWorkloadService {
    ...
    async getWorkspaceWorkload(
        ...
        const totalTasks = memberWorkloads.reduce((sum, m) => sum + m.assignedTasks, 0);
        const totalCompleted = memberWorkloads.reduce((sum, m) => sum + m.completedTasks, 0);
        const totalOverdue = memberWorkloads.reduce((sum, m) => sum + m.overdueTasks, 0);
        const averageWorkload = memberWorkloads.length > 0
            ? Math.round(memberWorkloads.reduce((sum, m) => sum + m.workloadScore, 0) / memberWorkloads.length)
            : 0;

        // Count by workload level
        const membersOverloaded = memberWorkloads.filter(m => m.workloadLevel === 'OVERLOADED').length;
        const membersUnderutilized = memberWorkloads.filter(m => m.workloadLevel === 'LOW').length;
        const membersBalanced = memberWorkloads.filter(m =>
            m.workloadLevel === 'MODERATE' || m.workloadLevel === 'HIGH'
        ).length;

        // Generate redistribution suggestions
        const redistributionSuggestions = this.generateRedistributionSuggestions(memberWorkloads);
    ...
    async getMemberWorkload(
        ...
        const hoursWorkedThisWeek = sessions.reduce(
    ...
    private calculateWorkloadScore(metrics: {
        ...
        const timeUtilization = (metrics.hoursWorkedThisWeek / this.WEEKLY_CAPACITY_HOURS) * 100;
     ...
     */
    private async calculateTrend(userId: string): Promise<MemberWorkload['trend']> {
        const now = new Date();
        const weekStart = this.getWeekStart();
    ...
    async getBalancingSuggestions(
        ...
        const membersWithOverdue = workload.members.filter(m => m.overdueTasks > 3);
