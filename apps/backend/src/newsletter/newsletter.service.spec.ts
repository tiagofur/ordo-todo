...
describe('NewsletterService', () => {
    ...
    describe('subscribe', () => {
        it('should create new subscription if not exists', async () => {
            ...
            prisma.newsletterSubscriber.create.mockResolvedValue({ id: '1', email: 'test@test.com', active: true });
        ...
        it('should throw BadRequest if already subscribed', async () => {
            prisma.newsletterSubscriber.findUnique.mockResolvedValue({ id: '1', email: 'test@test.com', active: true });

            await expect(service.subscribe({ email: 'test@test.com' })).rejects.toThrow(BadRequestException);
        });

        it('should reactivate if inactive', async () => {
            prisma.newsletterSubscriber.findUnique.mockResolvedValue({ id: '1', email: 'test@test.com', active: false });
            prisma.newsletterSubscriber.update.mockResolvedValue({ id: '1', email: 'test@test.com', active: true });
    ...
    describe('unsubscribe', () => {
        ...
        it('should throw NotFound if email not found', async () => {
            prisma.newsletterSubscriber.findUnique.mockResolvedValue(null);
            await expect(service.unsubscribe({ email: 'nope@test.com' })).rejects.toThrow(NotFoundException);
        });
    });
});
