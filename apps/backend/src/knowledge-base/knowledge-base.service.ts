...
export class KnowledgeBaseService {
    ...
    async findCategory(idOrSlug: string): Promise<KBCategory> {
        ...
        if (!category) {
            throw new NotFoundException(`Knowledge Base Category ${idOrSlug} not found`);
        }
        return category;
    }

    async updateCategory(id: string, dto: UpdateKBCategoryDto): Promise<KBCategory> {
    ...
    async findArticle(idOrSlug: string): Promise<KBArticle> {
        ...
        if (!article) {
            throw new NotFoundException(`Knowledge Base Article ${idOrSlug} not found`);
        }
        return article;
    }
