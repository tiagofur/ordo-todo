...
export class HabitsService {
    ...
    async create(createHabitDto: CreateHabitDto, userId: string) {
        this.logger.log(`Creating habit for user ${userId}: ${createHabitDto.name}`);
        ...
        const habit = await this.prisma.client.habit.create({
            data: {
                ...
                targetDaysOfWeek: createHabitDto.targetDaysOfWeek ?? [0, 1, 2, 3, 4, 5, 6],
        ...
        const habitCount = await this.prisma.client.habit.count({ where: { userId } });
        if (habitCount === 1) {
    ...
    async complete(id: string, completeDto: CompleteHabitDto, userId: string) {
        ...
        const completedAt = completeDto.completedAt ? new Date(completeDto.completedAt) : new Date();

        // Check if already completed today
        const existingCompletion = await this.prisma.client.habitCompletion.findUnique({
            where: {
        ...
        const yesterdayCompletion = await this.prisma.client.habitCompletion.findUnique({
            where: {
                habitId_completedDate: {
                    habitId: id,
    ...
    async uncomplete(id: string, userId: string) {
        ...
        await this.prisma.client.habitCompletion.delete({ where: { id: completion.id } });
    ...
    async getStats(id: string, userId: string) {
        ...
        const [thisWeekCompletions, thisMonthCompletions, last30Completions] = await Promise.all([
        ...
        for (let i = 0; i < 30; i++) {
            ...
            const completed = last30Completions.some(
                (c) => isSameDay(c.completedDate, date),
            );
            calendarData.push({
                date: dateStr,
                completed,
